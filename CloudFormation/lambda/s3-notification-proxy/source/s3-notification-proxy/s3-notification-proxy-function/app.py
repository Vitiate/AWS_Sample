...
def get_logstream(bucket, key):
    ...
    try:
        cloudwatch.create_log_stream(
            logGroupName=logGroupName, logStreamName=key)
    except cloudwatch.exceptions.ResourceAlreadyExistsException:
        pass
            ...
            try:
                ...
                if matcher:
                    ...
                    timestamp = get_timestamp(
                        log_fields[logTypeConfig['dateField']])
...
def json_log_parser(lines_read, bucket_name):
    ...
    for event_obj in records_obj:
        ...
        for path_obj in logTypeConfig['jsonPath']:
            value = get_json_value(
                event_obj, path_obj['key' if 'key' in path_obj else 'name'])
            if value:
                formatted_line[path_obj['key']] = value
                log_size += len(str(value))
        if not is_filters_matched(formatted_line):
            continue
        formatted_line['timestamp'] = int(get_timestamp(
            formatted_line[logTypeConfig['dateField']]))
...
def send_logs(parsedLines, logGroupName, logStreamName, logResponse):
    ...
    try:
        ...
        response = cloudwatch.put_log_events(
            logGroupName=logGroupName, logStreamName=logStreamName, logEvents=newEvents, sequenceToken=str(sequenceToken))
    except Exception as e:
        print(e)
        raise(e)
...
def lambda_handler(event, context):
    ...
    for tag in tags['TagSet']:
        ...
        if (tag['Key'] == "prefix:s3-notification-proxy"):
        ...
        if ("prefix:s3-notification-proxy:sqsrelay" in tag['Key']):
        ...
        if ("prefix:s3-notification-proxy:lambdarelay" in tag['Key']):
    ...
    if configTag is not None:
        ...
        logResponse, logGroupName, logStreamName = get_logstream(
        ...
        custom_regex = re.compile(
            logTypeConfig['regex']) if 'regex' in logTypeConfig else None
        ...
        if 'jsonPath' in logTypeConfig:
            ...
            parsed_lines, removed_log_size = parse_lines(
                lines_read.split(b'\n'), bucket)
        ...
        if parsed_lines:
            ...
            send_logs(parsedLines=parsed_lines, logGroupName=logGroupName,
                      logStreamName=logStreamName, logResponse=logResponse)
    return {
        'statusCode': 200,
